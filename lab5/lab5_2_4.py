...
K = np.array([k1, k2])
res = solve_ivp(deg1, [0,20], [2,0,0,0], rtol=1e-10, atol=1e-10)
...
plt.figure()
plt.title("Śledzenie trajektorii na podstawie estymat stanu - $u_{FB}=K(x_d-\hat{x})$")
plt.xlabel("t [s]")
plt.ylabel("U [v]")
plt.grid()
...
plt.xlim(10,20)
plt.ylim(0,5.5)
plt.title("Śledzenie trajektorii na podstawie estymat stanu - wykres od 10 sekundy")
plt.xlabel("t [s]")
plt.ylabel("U [v]")
plt.grid()
plt.plot(res.t, res.y[0], label="$x_1$")
